...
class EmployeeController extends BaseController{
    ...
    async addEmployee(req, res, validationResult) {
        try {
            if (validationResult[0]) throw new CustomError(validationResult[0].statusCode, validationResult[0].message, validationResult[0].status)
    ...
    @method("GET")
    @path("/get")
    @name('getEmployeeInfoById')
    async getEmployeeInfoById(req, res) {
        try {
            const id = req.query.id;
            if (!id) throw new CustomError(ResponseStatusCodes.BAD_REQUEST, ResponseMessages.ID_NOT_PROVIDE, ResponseStatus.ERROR)
            const result = await this.employeeSrv.get(id);
            if (!result.data || !result.parentId) throw new CustomError(ResponseStatusCodes.NOTFOUND, ResponseMessages.USER_NOT_FOUND, ResponseStatus.NOT_FOUND)
    ...
    async updateEmployeeInfo(req, res, validationResult) {
        try {
            ...
            if (!userExist(employee.data)) throw new CustomError(ResponseStatusCodes.NOTFOUND, ResponseMessages.USER_NOT_FOUND, ResponseStatus.NOT_FOUND)
